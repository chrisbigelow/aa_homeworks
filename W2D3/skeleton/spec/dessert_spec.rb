...
describe Dessert do
  let(:chef) { double(:name => "John") }
  subject(:cake) { Dessert.new("pastry", 45, chef) }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("pastry","",chef)}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      array_before = cake.ingredients.dup
      cake.mix!
      expect(cake.ingredients).not_to be(array_before)
    end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      ...
      expect(cake.serve.include?("Chef John the Great Baker")).to eq(true)
    end
  end
